    ...
    <main>
      ...
      <div className='content-page'>
        {props.workouts.sort((a,b)=>a.order-b.order).map((workout, i) => (
          <Link href={`./week5/${workout.link}`} key={i}>
            <h3>{workout.title}</h3>
          </Link>
        ))}
      </div>
...
export async function getStaticProps() {
  const folder = path.join(process.cwd(), './content/subscription/week5');
  const filenames = await fs.readdir(folder);
  const slugs = filenames.map(async s => {
    const content = (await fs.readFile(`./content/subscription/week5/${s}`)).toString();
